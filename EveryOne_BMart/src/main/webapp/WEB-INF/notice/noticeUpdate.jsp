		...
		<div class="col-sm-8">
			<div class="card bg-light" style="border: 0px">
				...
				<form action="update.nt" method="post" class="form-container">
					<input type="hidden" name="nnum" value="${notice.nnum}"> <input
						type="hidden" name="pageNumber" value="${pageNumber}"> <label
						for="noti">중요게시물 등록</label> <input type="radio" name="noti"
						value="y" required>Y <input type="radio" name="noti"
						value="n">N<br> <label for="category">카테고리:</label> <select
						id="category" name="category" required>
						<option value="공지사항"
							${notice.category == '공지사항' ? 'selected' : ''}>공지사항</option>
						<option value="FAQ" ${notice.category == 'FAQ' ? 'selected' : ''}>FAQ</option>
					</select> <br> <label for="subject">제목:</label> <input type="text"
						id="subject" name="subject" value="${notice.subject}" required>
					...
					<button type="button" class="plus_btn"
						onClick="location.href='list.nt?pageNumber=${param.pageNumber}'">돌아가기</button>
				</form>
