...
<div class="container my-5 p-4"
	...
	<div class="row">
		...
		<div class="col-sm-6 me-auto">
		    ...
		    <h1>주문 상세</h1>
    <p><strong>주문번호:</strong> ${order.onum}</p>
    <p><strong>고객 ID:</strong> ${order.id}</p>
    <p><strong>상품번호:</strong> ${order.pnum}</p>
    <p><strong>수량:</strong> ${order.pamount}</p>
    <p><strong>금액:</strong> ${order.price}</p>
    <p><strong>배송 방법:</strong> ${order.way}</p>
    <p><strong>상태:</strong> ${order.status}</p>
    <p><strong>내용:</strong> ${order.contents}</p>
    <p><strong>결제 방법:</strong> ${order.payment}</p>
    <p><strong>주문일:</strong> ${order.orderdate}</p>
<input type="button" value="돌아가기" onClick="location.href='list.od?pageNumber=${param.pageNumber}'">
