...
<div class="container-fluid">
	...
	<div class="row">
		...
		<form action="${pageContext.request.contextPath}/update.prd"
			...
			<label for="expritydate">유통기한:</label> <input type="text"
				...
				required> <label for="pcategory">카테고리:</label> <select
				id="pcategory" name="pcategory" required>
				<option value="쌀/잡곡/견과"
					${product.pcategory == '1' ? 'selected' : ''}>쌀/잡곡/견과</option>
				<option value="국/탕/찌개" ${product.pcategory == '2' ? 'selected' : ''}>국/탕/찌개</option>
				<option value="음료/커피/생수"
					${product.pcategory == '3' ? 'selected' : ''}>음료/커피/생수</option>
				<option value="정육/수산/계란"
					${product.pcategory == '4' ? 'selected' : ''}>정육/수산/계란</option>
				<option value="라면/면" ${product.pcategory == '5' ? 'selected' : ''}>라면/면</option>
				<option value="과일/채소" ${product.pcategory == '6' ? 'selected' : ''}>과일/채소</option>
				<option value="양념/장류/오일"
					${product.pcategory == '7' ? 'selected' : ''}>양념/장류/오일</option>
				<option value="우유/유제품" ${product.pcategory == '8' ? 'selected' : ''}>우유/유제품</option>
				<option value="햄/어묵/통조림"
					${product.pcategory == '9' ? 'selected' : ''}>햄/어묵/통조림</option>
