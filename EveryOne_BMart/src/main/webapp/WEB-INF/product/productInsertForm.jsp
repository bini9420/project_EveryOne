...
<script type="text/javascript">
	var s_selbox = new Array('전자기기', '가구/인테리어', '주방용품', '반려용품', '스포츠/레저/캠핑');

	function init(mc) {
		console.log("init 함수 호출됨, mc 값:", mc);
		
		document.myform.second.options[0] = new Option("카테고리", ""); // text, value
		for (i = 0; i < s_selbox.length; i++) {
			document.myform.second.options[i + 1] = new Option(s_selbox[i], s_selbox[i]);
			if (document.myform.second.options[i + 1].value == mc) {
				document.myform.second.options[i + 1].selected = true;
...
<body onLoad="init('${product.pcategory}')"> 
							...
							<table class="table table-borderless bsb-table-xl text-nowrap align-middle m-0">
								<tbody>
									<tr>
								        ...
								        <td>
								        	<select id="second" name="pcategory" style="width: 150px" class="form-select form-select-sm" required></select>
										</td>
							         </tr>
							         <tr>
							            <td>
